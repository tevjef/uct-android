...
public class SectionInfoVH extends RecyclerView.ViewHolder {
  ...
  public static SectionInfoVH newInstance(View parent) {
    ...
    CircleView sectionNumberBackground =
        ButterKnife.findById(parent, R.id.section_number_background);
  ...
  public void setOpenStatus(Section section) {
    if (section.status.equals("Open")) {
      mSectionNumberBackground.setBackgroundColor(ContextCompat.getColor(
          mParent.getContext(),
          R.color.green
      ));
    } else {
  ...
  public void setTimes(Section section) {
    ...
    for (int i = 0; i < mSectionTimeContainer.getChildCount(); i++) {
      ...
      if (time == null) {
        ...
        mSectionLocationText =
            (TextView) timeLayout.findViewById(R.id.section_item_time_info_location_text);
        mTimeText =
            (TextView) timeLayout.findViewById(R.id.section_item_time_info_meeting_time_text);
